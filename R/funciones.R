# Funciones paquete ilde ais AVM
...
#' @param disntancia distancia maxima a propiedades comparables. Default 500 metros
#' @keywords direccion geolocalizar 
#' @export
#' @examples
#' norm2("CALLE SECRETA 123 DP 1293 BOD 45")
reducir_base_comparar <- function(registro, df, distancia = 500, variable_metraje = "sum_V8_b", similitud = 0.2) {
  ...
  ind2 <- df_todos$region == registro$cod_region_habits & as.numeric(df$comuna) == registro$cod_comuna_habits
    ind3 <- ind2 & ind
  }
  ...
  indx <- vec < distancia
  df$distancias <- vec
  df_filt <- df_todos[indx & !is.na(indx),]
